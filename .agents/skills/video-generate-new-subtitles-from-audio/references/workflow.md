...
- A fresh run safely clears that directory and writes helpers under `output/<stem>/debug/video-generate-new-subtitles-from-audio/`. Random/hash suffixes and custom workspace ids are not supported.
...
- Keep audio in `output/<stem>/debug/video-generate-new-subtitles-from-audio/audio/`.
...
- Keep raw outputs in `output/<stem>/debug/video-generate-new-subtitles-from-audio/whisper/`.
...
- Keep the clean SRT under `output/<stem>/debug/video-generate-new-subtitles-from-audio/postprocess/`; it is an internal remux input.
...
- Validate that all embedded cue timestamps differ from their source SRT timestamps by one uniform mux shift (maximum spread 5 ms), and record the strategy and shift in `output/<stem>/debug/video-generate-new-subtitles-from-audio/reports/remux_validation.json`.
