...
7. Extract the selected audio stream to mono 16 kHz WAV in `output/<stem>/debug/video-text-agent-transcription/audio/`.
8. Run WhisperX or Whisper fallback with `--language <confirmed-code>` and `--output_format all`.
9. Keep all native backend formats in `output/<stem>/debug/video-text-agent-transcription/whisper/raw/` and clean auxiliary VTT/TXT in `whisper/postprocess/`.
