...
from mkv_subtitle_agentic_translation.workspace import build_workspace
from mkv_subtitle_agentic_translation.normalize_translation_maps import sanitize_map
from mkv_subtitle_agentic_translation.translation_maps import find_translation_maps
from mkv_subtitle_agentic_translation.translation_terms import apply_terms, load_term_maps
from mkv_subtitle_agentic_translation.workflow_checkpoint import (
    build_checkpoint,
...
class WorkspaceAndTermsTests(unittest.TestCase):
    def test_workspace_paths_share_same_output_name(self):
        workspace = build_workspace("input/NIPPON SANGOKU.mkv")
        ...
        self.assertIn("output/NIPPON SANGOKU/debug/mkv-subtitle-agentic-translation", debug_dir)
        self.assertIn("output/NIPPON SANGOKU/debug/mkv-subtitle-agentic-translation/subtitles", subtitles_dir)
        self.assertIn("output/NIPPON SANGOKU/debug/mkv-subtitle-agentic-translation/reports", reports_dir)
        self.assertIn("output/NIPPON SANGOKU/debug/mkv-subtitle-agentic-translation/translations", translations_dir)
        self.assertEqual(output_dir, "output/NIPPON SANGOKU")
        self.assertIn("output/NIPPON SANGOKU/debug/mkv-subtitle-agentic-translation/subtitles/source/NIPPON SANGOKU.source.ass", workspace["source_ass"].replace("\\", "/"))
        self.assertIn("output/NIPPON SANGOKU/debug/mkv-subtitle-agentic-translation/subtitles/generated/NIPPON SANGOKU.spa.ass", workspace["spanish_ass"].replace("\\", "/"))
        self.assertIn("output/NIPPON SANGOKU/debug/mkv-subtitle-agentic-translation/subtitles/generated/NIPPON SANGOKU.spa.srt", workspace["tv_safe_srt"].replace("\\", "/"))
        self.assertIn("output/NIPPON SANGOKU/debug/mkv-subtitle-agentic-translation/reports/spanish_normalization_report.json", workspace["normalization_report"].replace("\\", "/"))
        self.assertIn("output/NIPPON SANGOKU/debug/mkv-subtitle-agentic-translation/reports/remux_validation_report.json", workspace["remux_validation_report"].replace("\\", "/"))
        self.assertIn("output/NIPPON SANGOKU/NIPPON SANGOKU.spa.mkv", workspace["output_mkv"].replace("\\", "/"))
    ...
    def test_translation_checkpoint_records_exact_resume_context(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            debug_dir = root / "output" / "Movie" / "debug" / "mkv-subtitle-agentic-translation"
            source_ass = debug_dir / "source" / "Movie.source.ass"
            translations_dir = debug_dir / "translations"
            launcher = root / "src" / "mkv-subtitle-agentic-translation" / "traducir_subs_mkv.ps1"
    ...
    def test_translation_checkpoint_preserves_creation_time_across_status_updates(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            values = {
                "output_name": "Movie",
                "input_mkv": root / "input" / "Movie.mkv",
                "source_ass": root / "output" / "Movie" / "debug" / "mkv-subtitle-agentic-translation" / "source" / "Movie.source.ass",
                "source_language": "en",
                "source_language_name": "English",
                "source_codec": "ass",
                "stream_index": 3,
                "mkv_track_id": 4,
                "translations_dir": root / "output" / "Movie" / "debug" / "mkv-subtitle-agentic-translation" / "translations",
                "launcher": root / "src" / "mkv-subtitle-agentic-translation" / "traducir_subs_mkv.ps1",
            }
