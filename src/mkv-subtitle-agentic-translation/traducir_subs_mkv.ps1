...
$workspaceArgs = @(
    "-m", "mkv_subtitle_agentic_translation.workspace",
    "--input-mkv", $InputMkv,
    "--json"
)
...
$languageArgs = @(
    "-m", "mkv_subtitle_agentic_translation.subtitle_language",
    "--input-mkv", $InputMkv,
    "--json"
)
...
$null = Initialize-VideoOutputWorkspace `
    ...
    -DebugDir $DebugDir `
    -InputPath $InputMkv `
    -Workflow "mkv-subtitle-agentic-translation" `
    -Resume:$Resume
...
function Write-TranslationWorkflowCheckpoint {
    ...
    $checkpointArgs = @(
        "-m", "mkv_subtitle_agentic_translation.workflow_checkpoint",
...
if (!$PSBoundParameters.ContainsKey("TranslationJson")) {
    $mapResolverArgs = @(
        "-m", "mkv_subtitle_agentic_translation.translation_maps",
        "--translations-dir", $TranslationsWorkDir,
...
$sanitizedMapsJson = & $PythonExe -m mkv_subtitle_agentic_translation.normalize_translation_maps `
...
Invoke-Checked {
        & $PythonExe -m mkv_subtitle_agentic_translation.ass_apply_translations `
...
if ($TvSafeSrt) {
    ...
    Invoke-Checked {
        & $PythonExe -m mkv_subtitle_agentic_translation.ass_to_tv_safe_srt `
...
if ($TvSafeSrt -and !$SkipSpanishNormalization) {
    ...
    Invoke-Checked {
        & $PythonExe -m mkv_subtitle_agentic_translation.normalize_spanish_subtitles `
            --input-ass $SpanishAss `
            --input-srt $TvSafeSrt `
            --output-ass $SpanishAss `
            --output-srt $TvSafeSrt `
