...
$workspaceArgs = @(
    "-m", "video_subtitle_agentic_transcription.workspace",
...
$null = Initialize-VideoOutputWorkspace `
    ...
    -Workflow "video-subtitle-agentic-transcription"
...
$postprocessArgs = @(
    "-m", "video_subtitle_agentic_transcription.postprocess",
...
if (!$SkipRemux) {
    ...
    $remuxArgs = @(
        "-m", "video_subtitle_agentic_transcription.remux",
        "--input-video", $InputVideo,
        "--srt", $TranscribedSrt,
