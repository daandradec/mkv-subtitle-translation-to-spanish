...
from video_toolkit.subtitles.text import parse_text_subtitles
from video_toolkit.transcription.artifacts import find_backend_output, read_detected_language
...
def write_clean_text_outputs(cleaned_segments, public_output_dir, debug_output_dir, video_stem):
    public_output_dir = Path(public_output_dir)
    debug_output_dir = Path(debug_output_dir)
    public_output_dir.mkdir(parents=True, exist_ok=True)
    debug_output_dir.mkdir(parents=True, exist_ok=True)
    public_outputs = {
        "srt": public_output_dir / f"{video_stem}.srt",
    }
    debug_outputs = {
        "vtt": debug_output_dir / f"{video_stem}.vtt",
        "txt": debug_output_dir / f"{video_stem}.txt",
    }
    write_clean_srt(cleaned_segments, public_outputs["srt"])
    write_clean_vtt(cleaned_segments, debug_outputs["vtt"])
    write_clean_txt(cleaned_segments, debug_outputs["txt"])
    return (
        {key: str(path) for key, path in public_outputs.items()},
        {key: str(path) for key, path in debug_outputs.items()},
    )
...
def postprocess_text_transcription(
    ...
    output_dir = Path(output_dir)
    raw_output_dir = Path(raw_output_dir or output_dir)
    copied = canonicalize_native_outputs(raw_output_dir, audio_stem, video_stem)
    segments, detected_language, raw_json = load_segments(
        raw_output_dir,
        audio_stem,
    ...
    public_outputs, debug_outputs = write_clean_text_outputs(
        cleaned,
        output_dir,
        debug_output_dir,
        video_stem,
...
def main():
    ...
    if args.json:
        ...
        print(f"Markdown: {report['public_outputs']['markdown']}")
