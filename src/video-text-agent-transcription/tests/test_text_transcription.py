...
from video_text_agent_transcription.postprocess import clean_segment_text, postprocess_text_transcription, validate_markdown_paragraphs
from video_text_agent_transcription.workspace import build_text_transcription_workspace
...
class TextTranscriptionTests(unittest.TestCase):
    def test_workspace_uses_existing_pattern_and_outputs_markdown(self):
        ...
        self.assertIn("output/capacitacion ma/debug/video-text-agent-transcription/audio", workspace["audio_wav"].replace("\\", "/"))
        self.assertIn("output/capacitacion ma/debug/video-text-agent-transcription/whisper/raw", workspace["json"].replace("\\", "/"))
        self.assertIn("output/capacitacion ma/debug/video-text-agent-transcription/whisper/postprocess", workspace["vtt"].replace("\\", "/"))
