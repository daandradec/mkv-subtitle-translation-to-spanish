...
$workspaceArgs = @(
    "-m", "video_voice_cleaner.voice_cleaner",
    "workspace",
...
if ($LASTEXITCODE -ne 0) {
    throw "Voice cleaner output path generation failed."
}
...
$chainJson = & $PythonExe -m video_voice_cleaner.voice_cleaner `
    "chain" `
    "--profile" $Profile `
...
try {
    ...
    $statsJson = & $PythonExe -c "import json, pathlib; from video_voice_cleaner.voice_cleaner import extract_loudnorm_json; print(json.dumps(extract_loudnorm_json(pathlib.Path(r'$loudnormPass1Log').read_text(encoding='utf-8-sig')), ensure_ascii=False, indent=2))"
    if ($LASTEXITCODE -ne 0) {
    ...
    $secondPassJson = & $PythonExe -m video_voice_cleaner.voice_cleaner `
